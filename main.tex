...
This template satisfies the Cal Poly Grad Ed M.S.\ Thesis formatting
requirements (2022--2023 guidelines, the most recent at time of
writing). Always run a final draft past the thesis editor before
submission, since the requirements are revised periodically. You should
not need to modify the document preamble unless your thesis requires
extra packages (theorem environments, \texttt{tikz}, \texttt{algorithm2e},
table customization, etc.). Drafting is otherwise plug-and-play: write
your chapters, swap in your front-matter declarations, and add or
remove the appendices.

The remainder of this chapter exercises the LaTeX features you are
likeliest to need---math, figures, tables, and citations---so you can
see them rendered in context. Sections within a chapter use
\verb!\section{...}!, \verb!\subsection{...}!, and
\verb!\subsubsection{...}!; the document preamble shows two depths in
the table of contents, but all three are numbered. If you need an
unnumbered section, use the starred form (e.g.\ \verb!\subsection*{}!),
but do not cross-reference unnumbered sections.

\section{Math}\label{sect-math}

For inline math, wrap the expression in single dollar signs, e.g.\
$Y = X\beta + \epsilon$. For a numbered display equation, use the
\verb!equation! environment:
\begin{equation}\label{eqn-lm}
    Y = X\beta + \epsilon.
\end{equation}
Use the \verb!equation*! variant for an unnumbered display, and number
only those equations you cross-reference (such as
Equation~\ref{eqn-lm}). Equations are numbered consecutively by chapter
automatically.

For multi-line derivations, use the \verb!align*! environment:
\begin{align*}
    y_i &= \beta_0 + x_i^\top\beta + \epsilon_i \\
        &= \beta_0 + \sum_{j=1}^{p} x_{ij}\beta_j + \epsilon_i.
\end{align*}
Drop the asterisk and add labels if you need to reference individual
lines. \citet{latex-math} is a useful general reference. One small
tip: prefer scalable delimiters via \verb!\left(...\right)!, which
makes a noticeable difference in display style.

\section{Figures and tables}

Figures live in the \verb!img/! directory (or anywhere else in the
project). Insert them with \verb!\includegraphics!:
\begin{figure}[h!]
    \centering
    \includegraphics[width=0.5\linewidth]{img/normal-dist.png}
    \caption{The normal distribution, rendered for purely
             demonstrative reasons.}
    \label{fig-normaldist}
\end{figure}
Figure captions go below the figure. Tables are typeset directly in
LaTeX, with the caption above:
\begin{table}[h!]
    \centering
    ...
    \end{tabular}
\end{table}
For more on table layout, see \citet{latex-tables}. The R package
\verb!xtable! \citep{xtable} can convert data frames to LaTeX tables;
combined with \verb!clipr!, it makes table generation painless.

\section{Citations}

The file \verb!refs.bib! contains examples of the most common BibTeX
entry types: articles, books, book chapters, R packages, and web
resources. Google Scholar generates these entries automatically, but
review them for consistent capitalization and missing fields before
committing them. Only cited entries appear in the bibliography.

Use \verb!\citet! for textual citations:
\citet{bibtex} provides a useful introduction to reference management
in BibTeX. Use \verb!\citep! for parenthetical citations: stability
selection is a general method of variable selection with theoretical
guarantees on the expected number of false discoveries
\citep{meinshausen2010stability}. A list of works is comma-separated
within one \verb!\citep!: envelope methods improve estimation efficiency
in multivariate models by exploiting low-dimensional structure
\citep{cook2010envelope, cook2013envelopes, cook2015foundations}.
Citation appearance is controlled by options to the \verb!natbib!
package; see \citet{natbib-guide} for the option list.
